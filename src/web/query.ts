...
async function remove_login(id: string) {
	if (!window.confirm("Are you sure you want to remove this login?")) { return; }
	...
	if (res.ok) {
		...
		if (window.localStorage.getItem("show-remove-success") == null ) {
			window.localStorage.setItem("show-remove-success", String(window.confirm("Successfully removed a login. Would you like to show these messages in the future?")));
		}
		else if (window.localStorage.getItem("show-remove-success") == "true") {
